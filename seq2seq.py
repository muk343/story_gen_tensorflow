...
@tf.function
def batch_eval(inp, targ, enc_hidden):
  loss = 0
...
for epoch in range(EPOCHS):
  ...
  """
  ####################################################################################
  print("Now, going for val data!!!")

  enc_hidden = encoder.initialize_hidden_state()
  total_loss = 0

  for (batch, (inp, targ)) in enumerate(dataset_val.take(steps_per_epoch)):
    batch_loss = batch_eval(inp, targ, enc_hidden)
    total_loss += batch_loss
...
def plot_attention(attention, sentence, predicted_sentence):
    ...
    plt.savefig("attention_weights_" + str(random.random()) + ".png")
